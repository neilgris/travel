...
- Create: `app/templates/trips/detail.html`（本任务先建**最小占位版**，Task 11 扩展为完整版）
...
> ⚠️ 计划修正：本任务的 `test_create_trip` 用 `follow_redirects=True`，POST 后会跳到 `trips.detail` 并渲染 `trips/detail.html`。该模板原计划在 Task 11 才创建，会导致本任务测试 `TemplateNotFound`。因此本任务先创建一个**最小占位 `detail.html`**（仅标题与总花费），Task 11 改为**扩展/重写**它（加入每天、记录、表单等）。
...
- [ ] **Step 5: 建最小占位 detail.html**

`app/templates/trips/detail.html`（最小版，Task 11 扩展）:
```html
{% extends "base.html" %}
{% block title %}{{ trip.title }}{% endblock %}
{% block content %}
<h1>{{ trip.title }}</h1>
<p>{{ trip.start_date }} ~ {{ trip.end_date }} · 总花费 ￥{{ stats.total_cny }}</p>
{% endblock %}
```

> `detail` 路由本任务已渲染该模板（传入 `trip` 和 `stats=trip_stats(trip)`）。Task 11 会把它扩展为含每天/记录/表单的完整版。
...
**Files:**
- Modify: `app/templates/trips/detail.html`（Task 10 的最小占位 → 扩展为完整版）
...
> ⚠️ 计划修正（必做）：完整版 detail.html 含 `url_for('trips.stats_page', ...)`。本任务的 `test_add_day_and_entry` 用 `follow_redirects=True`，会渲染 detail.html，从而在渲染时调用 `url_for('trips.stats_page')`——若该端点未注册会抛 `BuildError`。因此**本任务必须先加一个占位 `stats_page` 路由**（只需端点存在即可让 `url_for` 构建 URL；Task 12 再替换为完整实现）。

- [ ] **Step 5: 加占位 stats_page 路由（让 detail.html 的 url_for 可解析）**
...
**Files:**
- Modify: `app/blueprints/trips.py`（把 Task 11 的占位 `stats_page` 替换为完整实现）
