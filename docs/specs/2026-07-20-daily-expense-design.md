...
- 每项右侧带累计金额；进页面与每次切「分类 / 标签」都自动选中**当前列表最上面那一项**并滚到可见处——所见即所选，不去替用户找全局金额最大的那个（分类模式下最上面的本来就是金额最高的一级；标签模式按标签组排，头一项是最大那组的头名）
- 满宽折线画该维度逐年金额（X 轴为全量记账跨度 first_year..last_year，某年无消费画 0；各维度共用一条时间轴）
