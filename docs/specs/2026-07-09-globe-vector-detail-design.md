...
| Tier 1（详情/放大） | `zoom ≥ DETAIL_ZOOM` | 精细海岸线填充 + 国界&省/州界 | 新 `land-10m.json` + `admin1-10m.json` |
...
- **懒加载**：`land-10m.json`、`admin1-10m.json` 仅在 `zoom` **首次**越过 `DETAIL_ZOOM` 时 `fetch`，
  不影响首屏。加载入口放在 `globe-home.js` 的 `ensureDeps`（或等价钩子），结果存进 `shared.landHi` / `shared.admin1`，
  **仅 D3 渲染器消费**，gl/sat 不受影响。
- **绘制切换**：`globe-d3.js` 的 `draw()` 按当前 `zoom` 选层。
  - `zoom < DETAIL_ZOOM`：走 Tier 0（现逻辑）。
  - `zoom ≥ DETAIL_ZOOM`：用 `shared.landHi` 填充 + 画 `shared.admin1` 的 mesh。
...
- `globe-home.js`：`ensureDeps` 增加详情层懒加载分支；`shared` 增加 `landHi` / `admin1` 字段。
- `globe-d3.js`：`draw()` 按 zoom 分支选层；`wheel` / `flyTo` 缩放路径里，跨阈值时触发懒加载。
